            ...
            if (xhr.status === 200) {
                ...
                for (var key in response.comments) {
                    if (response.comments[key].player_id2 == id) {
                        var temp = '<div class="comment_holder2"><p style="color:dodgerblue"><i style="color:yellowgreen" class="fa fa-user-circle"></i>' + response.comments[key].username2 + '</p><p> <small style="font-size:80%; text-align:justify;">' + response.comments[key].comment + '</small> </p></div>';
                        commentBody.innerHTML += temp;
                    }
                    else {
                        var temp = '<div class="comment_holder"><p style="color:dodgerblue"><i style="color:slateblue" class="fa fa-user-circle"></i> <small>' + response.comments[key].username2 + '</small></p><p> <small style="color:font-size:80%; text-align:justify;">' + response.comments[key].comment + '</small> </p></div>';
                        commentBody.innerHTML += temp;
                    }
                }
            } else {
        ...
        xhr1.onload = function () {
            if (xhr1.status === 200) {
                ...
                for (var key in response.mydata) {
                    var temp = '<p>' + response.mydata[key].firstname + " " + response.mydata[key].lastname + '</p>'
            ...
            if (xhr3.status === 200) {
                ...
                for (var key in response.results) {
                    if (response.results[key].play_id == id) {
                        var temp = '<tr style="background-image:linear-gradient(to top right,#020412 20%, #383235 40%,#04071c 70%);"><td>' + x++ + '</td><td>' + response.results[key].username + '</td><td>' + response.results[key].wpm + '</td><td>' + response.results[key].cpm + '</td><td>' + response.results[key].mistakes + '</td></tr>';
                        res_Body.innerHTML += temp;
                    }
                    else {
                        var temp = '<tr><td>' + x++ + '</td><td>' + response.results[key].username + '</td><td>' + response.results[key].wpm + '</td><td>' + response.results[key].cpm + '</td><td>' + response.results[key].mistakes + '</td></tr >';
                        res_Body.innerHTML += temp;
                    }
                }
