...
const save_data2 = document.getElementById("save_data2");
const save_data3 = document.getElementById("save_data3");
...
const pass1 = document.getElementsByClassName("edit1")[4];
const pass2 = document.getElementsByClassName("edit1")[5];
const pass3 = document.getElementsByClassName("edit1")[6];
...
save_data.addEventListener("submit", function (e) {
  ...
  fetch("/second_player_data", {
    ...
    .then((responseText) => {
      ...
      document.querySelector("#notText").innerHTML = responseText;
      notificationsmob.style.top = "2px";
      notText2.innerHTML = responseText;
...
save_data3.addEventListener("submit", function (e) {
  e.preventDefault();

  var passone = pass1.value;
  var passtwo = pass2.value;
  var passthree = pass3.value;
  ...
  const csrfToken4 = document.querySelector("#csrf_token4b").value;
  fetch("/updatepass", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    .then((responseText) => {
      ...
      notText2.innerHTML = responseText;
      setTimeout(closeNotification, 5000);
      if (responseText == "You have successfully changed the password.") {
        pass1.value = "";
        pass2.value = "";
        pass3.value = "";
...
save_data2.addEventListener("submit", function (e) {
  e.preventDefault();
  ...
  if (selectedImage) {
    ...
    reader.onload = function (event) {
      ...
      img.onload = function () {
        ...
        const csrfToken4 = document.querySelector("#csrf_token4a").value;
        XHR3.open("POST", "/upload_PIC", true);
        ...
        XHR3.addEventListener("load", function () {
          if (XHR3.status === 200 && XHR3.readyState === 4) {
            ...
            notifications.style.width = "40%";
            notifications.style.opacity = "1";
            document.querySelector("#notText").innerHTML = response;
            window.location.reload();
            notificationsmob.style.top = "2px";
            notText2.innerHTML = response;
            setTimeout(closeNotification, 5000);
          } else {
            notifications.style.width = "40%";
            notifications.style.opacity = "1";
    ...
    notifications.style.width = "40%";
    notifications.style.opacity = "1";
    document.querySelector("#notText").innerHTML = "Please select an image.";
    notificationsmob.style.top = "2px";
    notText2.innerHTML = "Please select an image.";
    setTimeout(closeNotification, 5000);
  }
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const retriveComments = setInterval(function () {
    const firstIdOb = { id };
    const jsonData = JSON.stringify(firstIdOb);
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "/get_comments", true);
    const csrfToken = document.querySelector("#csrf_tokena3").value;
...
function get_user_profile() {
  ...
  xhr1.open("POST", "/get_my_data", true);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateLeaderboard() {
    ...
    xhr3.open("POST", "/get_test_details", true);
...
function updateHistory() {
  ...
  const firstIdOb = { id };
  const jsonData = JSON.stringify(firstIdOb);
  xhr5.open("POST", "/get_history", true);
  const csrfToken = document.querySelector("#csrf_tokena5").value;
  xhr5.setRequestHeader("Content-Type", "application/json");
  xhr5.setRequestHeader("X-CSRFToken", csrfToken);
  xhr5.onload = function () {
    if (xhr5.status === 200) {
      const contentType = xhr5.getResponseHeader("Content-Type");
...
function showWinner() {
  ...
  var xhr3 = new XMLHttpRequest();
  xhr3.open("POST", "/leaderBoardHistory", true);
...
function showLooser() {
  ...
  xhr3.open("POST", "/leaderBoardHistory2", true);
      ...
      if (contentType && contentType.includes("application/json")) {
        ...
        for (var key in response.results) {
          ...
          document.querySelector("#winnerRemark2").innerHTML =
            "WE ARE SORRY TO ANNOUNCE THAT YOU DIDN'T MAKE IT TO THE TOP..";
          var temp =
            ' <div class="winnerOutput"><div class="winerImg"> <img src="' +
            response.results[key].profile_pic +
            '" alt="image" style="width: 120px; height: 120px; border-radius: 50%"/></div><p style="color: gray; text-align: center; font-weight:600">' +
        ...
        if (xhr3.responseText.length > 920) {
          document.querySelector("#winnerRemark").innerHTML =
            "YOU DID'NT PARTCIPATE LAST EVENT";
        } else {
          document.querySelector("#winnerRemark").innerHTML =
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  setInterval(function () {
    ...
    xhr.open("POST", "/get_concern2", true);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.setRequestHeader("X-CSRFToken", csrfToken);
    xhr.onload = function () {
      if (xhr.status === 200) {
